...
async def test_strictly_idempotent_execution_under_concurrent_load(db_session: AsyncSession, client: TestClient):
    ...
    db_session.add(User(id=user_id, email="test@faang.com", password_hash="hash", tenant_config={}))
    db_session.add(Pipeline(id=pipeline_id, name="Chaos Pipe", user_id=user_id, steps=[]))
    db_session.add(Dataset(id=dataset_id, user_id=user_id, name="chaos.csv", s3_key="x", row_count=10))
    await db_session.commit()

    with patch('app.services.reliability.execute_pipeline_task.apply_async') as mock_apply:
